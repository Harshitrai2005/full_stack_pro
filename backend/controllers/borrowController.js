...
export const recordBorrowedBooks=catchAsyncErrors(async (req,res,next)=>{
    ...
    if(isAlreadyBorrowed){
        return next(new ErrorHandler("Book is alredy borrowed"),404);
    }    
...
export const returnBorrowedBook =catchAsyncErrors(async (req,res,next)=>{
     ...
     if(!borrow){
        return next(new ErrorHandler("Book is not borrowed",404));
     }
     ...
     res.status(200).json({
        success:true,
        message:fine!==0?`The book has been returned successfully.The total charges including fine are ${fine+book.price}`
